...
function GetLatestScanResult {
    ...
    try {
        $latestScanResultEndpoint = "$fortifySscUrl/api/v1/projectVersions/2392225302/issues"
        $headers = @{
...
function GetSeverityValue {
    ...
    $severityValue = switch ($severityValue) {
        "Low" { return 1 }
        "Medium" { return 2 }
        "High" { return 3 }
        Default { return 4 }
...
#Step 5 
$ciaApplicationId = GetApplicationId -fortifySscUrl $fortifySscUrl  -applicationName $applicationName -accessToken $accessToken 
$applicationVersion = $ciaApplicationId.projects.versions | Sort-Object -Property id  | Select-Object -Last 1
if ($ciaApplicationId) {
    $latestScanResult = GetLatestScanResult -applicationId $ciaApplicationId.id -version $applicationVersion.id -fortifySscUrl $fortifySscUrl -accessToken $accessToken
